...
describe('writing section', () => {
  ...
  it('the nav has a Writing link pointing at #writing', () => {
    const link = Array.from(document.querySelectorAll('.nav-link')).find(a => a.textContent.trim() === 'Writing');
    expect(link).toBeTruthy();
    expect(link.getAttribute('href')).toBe('#writing');
  });

  it('renders an empty grid without erroring when essays.json is missing', async () => {
    mockFetch({});
    await window.loadEssays().then(() => window.renderWriting());
    expect(document.querySelectorAll('.essay-card').length).toBe(0);
  });

  it('renders one card per essay, showing genre/date/title/excerpt', async () => {
    mockFetch({
      'writing/essays.json': [
        {
    ...
    expect(card.querySelector('.essay-card-title').textContent).toBe('What the Light Skips Over');
    expect(card.querySelector('.essay-genre').textContent).toBe('Personal Essay · 2026');
    expect(card.querySelector('.essay-excerpt').textContent).toBe('A short teaser.');
  });

  it('clicking a card opens the essay with all paragraphs rendered', async () => {
    mockFetch({
      'writing/essays.json': [
        {
    ...
    await window.loadEssays().then(() => window.renderWriting());
    document.querySelector('.essay-card').dispatchEvent(new window.Event('click', { bubbles: true, cancelable: true }));

    const modal = document.getElementById('essay-modal');
    expect(modal.classList.contains('active')).toBe(true);
    expect(document.getElementById('essay-title').textContent).toBe('Two Paragraphs');
    const paragraphs = document.querySelectorAll('#essay-body p');
    expect(paragraphs.length).toBe(2);
    expect(paragraphs[0].textContent).toBe('First paragraph.');
    expect(paragraphs[1].textContent).toBe('Second paragraph.');
  });

  it('an essay with a matching photoTitle shows that photo; one without shows none', async () => {
    mockFetch({
      'writing/essays.json': [
        { title: 'Paired', genre: 'Reflection', excerpt: '...', body: ['...'], photoTitle: 'Praying Hands' },
        { title: 'Unpaired', genre: 'Reflection', excerpt: '...', body: ['...'] },
      ],
    });
    await window.loadEssays().then(() => window.renderWriting());

    window.openEssay(0);
    expect(document.getElementById('essay-photo-wrap').style.display).toBe('block');
    expect(document.getElementById('essay-photo').getAttribute('src')).toBe('PrayingHands.JPG');

    window.openEssay(1);
    expect(document.getElementById('essay-photo-wrap').style.display).toBe('none');
  });

  it('an essay with a photoTitle that matches no real photo shows no photo, and does not throw', async () => {
    mockFetch({
      'writing/essays.json': [
        { title: 'Bad Pairing', genre: 'Reflection', excerpt: '...', body: ['...'], photoTitle: 'Nonexistent Photo' },
      ],
    });
    await window.loadEssays().then(() => window.renderWriting());
    expect(() => window.openEssay(0)).not.toThrow();
    expect(document.getElementById('essay-photo-wrap').style.display).toBe('none');
  });

  it('the essay modal can be dismissed via close button, backdrop, or Escape', async () => {
    mockFetch({
      'writing/essays.json': [{ title: 'X', genre: 'Reflection', excerpt: '...', body: ['...'] }],
    });
    await window.loadEssays().then(() => window.renderWriting());
    const modal = document.getElementById('essay-modal');

    window.openEssay(0);
    document.querySelector('#essay-modal .modal-close').dispatchEvent(new window.Event('click', { bubbles: true, cancelable: true }));
    expect(modal.classList.contains('active')).toBe(false);
